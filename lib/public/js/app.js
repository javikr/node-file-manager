...
FMApp.controller('FileManagerCtr', ['$scope', '$http', '$location',
  function ($scope, $http, $location) {
    ...
    FM.zipFile = function (newName) {
      var folder = FM.selection[0];
      console.log('zip ', newName);
      httpRequest('PUT', 'zip', {type: 'ZIP'}, {folder: folder, zipname: newName});
    };
    ...
    FM.addSpotifyDownload = function (spotifyUri) {
      httpRequest('POST', 'addSpotifyDownload', {uri: spotifyUri}, null);
    };
    ...
    FM.btnDisabled = function (btnName) {
      switch (btnName) {
        ...
        case 'zip':
          return (FM.selection.length !== 1 || FM.selection[0].folder == false);
        case 'rename':
